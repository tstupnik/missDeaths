...
# observable survival
########################
my.survnp <- function(tt,st, D,ratetable,konc,conf.int=0.95){
  md.init(ratetable)
  ...
  test = rep(NA,length(ttun))
  
  for(it in 1:length(ttun)){
    #population survival probabilities
    Spi <- md.expprep(D, difft[it])			#populacijska verjetnost, da zivijo se naprej
